...
cv::Mat fsiv_convolve(cv::Mat const& in, cv::Mat const& filter, const bool circular=false)
{
    ...
    cv::Mat aux, salida;
    	...
    	for(int i = 1; i <= in.rows; i++)
    	{
    		...
    		{
    			for(int k = filter.rows/2; k > 0; k--)
    			{
    				aux = filter;
    				std::cout<<k<<std::endl;
    				aux.at<float>(k,k) *= in.at<float>(i,j);
    				//std::cout<<"prueba"<<std::endl;
    				aux.at<float>(k - 1,k) *= in.at<float>(i - 1,j);
    				//std::cout<<"prueba"<<std::endl;
    				aux.at<float>(k,k - 1) *= in.at<float>(i,j - 1);
    				//std::cout<<"prueba"<<std::endl;
    				aux.at<float>(k + 1,k + 1) *= in.at<float>(i + 1,j + 1);
    				//std::cout<<"prueba"<<std::endl;
    				aux.at<float>(k - 1,k - 1) *= in.at<float>(i - 1,j - 1);
    				//std::cout<<"prueba"<<std::endl;
    				aux.at<float>(k + 1,k) *= in.at<float>(i + 1,j);
    			...
    			std::cout<<cv::sum(aux)[0]<<std::endl;
    			salida.at<float>(i,j) = cv::sum(aux)[0];
    			std::cout<<"prueba"<<std::endl;
    		}
    	}
                         ...
                         int filter_type=0, bool circular=false)
{
   ...
   // }
    ...
    cv::Mat convolution = fsiv_convolve(in, filter, false);

    cv::Mat resultado = (g + 1)*in - (g*convolution);
