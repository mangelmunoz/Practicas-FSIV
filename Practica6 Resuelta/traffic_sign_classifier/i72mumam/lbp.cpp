...
static const float uniform[256] =
{
...
{    
    ...
    for(int i = 1; i < (img.rows - 1); i++)
    {
        for(int j = 1; j < (img.cols - 1); j++)
        {
            ...
            float value = img.at<float>(x,y); 
            float aux; 
            uchar result = 0; 

            aux = img.at<float>(i-1,j);
            if(aux > value){
                result = result | (1 << 7);
            }
            aux = img.at<float>(i-1,j-1);
            if(aux > value){
                result = result | (1 << 6);
            }

            aux = img.at<float>(i,j-1);
            if(aux > value){
                result = result | (1 << 5);
            }
...
void fsiv_lbp_desc(const cv::Mat & lbp, cv::Mat & lbp_desc, const int *ncells, bool normalize, bool uLBP)
{
    ...
    assert(ncells!=nullptr && (ncells[0]*ncells[1] > 0));
     ...
     int cell_h = cvFloor(double(lbp.rows) / ncells[0]);
     int cell_w = cvFloor(double(lbp.cols) / ncells[1]);
        ...
        {   
            for (int j = 0; j <= lbp.cols - cell_w; j = j + cell_w)
            {
                ...
                contador++;
                    ...
                    {
                        Theight = (lbp.rows - i - cell_h);
                    }

                    if(lbp.rows - j < 2 * cell_w && lbp.cols - j < cell_w)
                    {
                        Twidth = (lbp.cols - j - cell_w);
                    }
                ...
                fsiv_lbp_hist(trozolbp, trozohist, normalize);
                    ...
                    {
                    cv::hconcat(lbp_desc, trozohist, lbp_desc);
                    }
            }
        }

        lbp_desc /= (lbp_desc.rows ^lbp_desc.cols);

        // std::cout<<lbp_desc.cols<<std::endl;
        // std::cout<<(256*ncells[0]*ncells[1])<<std::endl;
    ...
    assert(!normalize || std::abs(cv::sum(lbp_desc)[0]-1.0)<1.0e6);
}
